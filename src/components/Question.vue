<template>
  <div class="jumbotron mt-3 py-3">
    ...
    <hr class="my-2" />
    <!-- 选项 -->
    <div v-for="(item, index) in currentQuestion.items" :key="index" class="form-check my-3">
      <label class="form-check-label">
        ...
        />
        <span class="badge badge-pill badge-danger mr-2">{{itemIndexs[index]}}</span>
        {{item}}
      </label>
    </div>
    <!-- 按钮 -->
    <div class="row">
      <div class="col-lg-6 col-sm-12">
        <!-- 按钮是否可用 -->
        <button @click="prev" type="button" class="btn btn-danger btn-block">上一题</button>
      </div>
      <div class="col-lg-6 col-sm-12">
        <button
          v-if="isLast"
